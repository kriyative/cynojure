...
(deftest test-sql
  ...
  (is (= (insert-into :report-p-daily-d
                      ...
                      (select '(:p-id [:count 1])
                              ...
                              :where [:and
                                      ...
                                      [:< :ctime 3468556800]]
                              :group-by :p-id))
         "INSERT INTO report_p_daily_d (p_id,d) VALUES (SELECT p_id,COUNT(1) FROM audit_log_entry WHERE ((o_id = 1) AND (ctime >= 3468470400) AND (ctime < 3468556800)) GROUP BY p_id)"))
  (is (= (insert-into :report-p-daily-l2
                      ...
                      (select '(:p-id [:count 1])
                              ...
                              :group-by :p-id))
         "INSERT INTO report_p_daily_l2 (p_id,l2) VALUES (SELECT p_id,COUNT(1) FROM audit_log_entry WHERE ((o_id = 1) AND (ctime >= 3468470400) AND (ctime < 3468556800) AND ((user_agent LIKE 'Mozilla%') OR (user_agent LIKE 'Opera%'))) GROUP BY p_id)"))
  (is (= (insert-into :report-p-daily-s
                      ...
                      (select '(:foo.p-id [:count 1])
                              :from (as (select '(:p-id :ip-address)
                                        ...
                                        :foo)
                              :group-by :foo.p-id))
         "INSERT INTO report_p_daily_s (p_id,s) VALUES (SELECT foo.p_id,COUNT(1) FROM (SELECT p_id,ip_address FROM audit_log_entry WHERE ((o_id = 1) AND (ctime >= 3468470400) AND (ctime < 3468556800) AND NOT(((user_agent LIKE 'Mozilla%') OR (user_agent LIKE 'Opera%')))) GROUP BY p_id,ip_address) as foo GROUP BY foo.p_id)"))
  ...
  (is (= (insert-into :actor
                      ...
                      [["firstname" "surname" "password"]
                       ["firstname" "surname" "password"]]
                      :returning [:a :b])
         "INSERT INTO actor (firstname,surname,password) VALUES ('firstname','surname','password'), ('firstname','surname','password') RETURNING (a,b)"))
  )
